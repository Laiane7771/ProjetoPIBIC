package edu.ifg.formosa.administrador.client.util;
...
public class FlexTableEventIF extends Composite{
	...
	public void prencheCabecalho(ArrayList<Widget> t){
		...
		while(itColuna.hasNext()){
			...
			ft.getCellFormatter().addStyleName(0, contColuna, "ftTabelaCampus");
			contColuna++;
		}
	...
	public void preencheTabela(ArrayList<ArrayList<Widget>> t){
		...
		while(itLinha.hasNext()){
			...
			while(itColuna.hasNext()){
				...
				ft.getCellFormatter().addStyleName(contLinha, contColuna, "ftTabelaCampus");
				contColuna++;
			}
			contLinha++;
			contColuna=0;
